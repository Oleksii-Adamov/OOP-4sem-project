...
CREATE UNIQUE INDEX `UserID_UNIQUE` ON `User`(`UserID`);
...
CREATE UNIQUE INDEX `UserID1_UNIQUE` ON `Authorization`(`UserID`);
...
CREATE UNIQUE INDEX `RoomID_UNIQUE` ON `Class`(`ClassID`);
CREATE INDEX `fk_Class_User_idx` ON `Class`(`TeacherUserID`);
...
CREATE TABLE IF NOT EXISTS `Assignment` (
  ...
  `AssignmenCreationDate` DATETIME NOT NULL,
  `AssignmentData` VARCHAR(66) NOT NULL,
  FOREIGN KEY (`TeacherUserID`) REFERENCES `User`(`UserID`) ON UPDATE CASCADE ON DELETE CASCADE);
CREATE UNIQUE INDEX `TaskID_UNIQUE` ON `Assignment`(`AssignmentID`);
CREATE INDEX `fk_Assignment_User1_idx` ON `Assignment`(`TeacherUserID`);
...
CREATE INDEX `fk_User_has_Class_User1_idx` ON `Student_Class`(`StudentUserID`);
CREATE INDEX `fk_User_has_Class_Class1_idx` ON `Student_Class`(`ClassID`);
...
CREATE TABLE IF NOT EXISTS `Student_Assignment` (
  ...
  FOREIGN KEY (`StudentUserID`) REFERENCES `User`(`UserID`) ON UPDATE CASCADE ON DELETE CASCADE);
CREATE INDEX `fk_Assignment_has_User_User1_idx` ON `Student_Assignment`(`StudentUserID`);
CREATE INDEX `fk_Assignment_has_User_Assignment1_idx` ON `Student_Assignment`(`AssignmentID`);
